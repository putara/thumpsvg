...
public:
    ...
    BOOL OnCreate(HWND hwnd, LPCREATESTRUCT lpcs)
    {
        ...
        SIZE size = this->toolbar
            .SetButtons(IDB_TOOLBAR32, IDR_MAIN)
            .Check(ID_VIEW_ZOOM_FIT, true)
            .GetMaxSize();
        ...
        this->manager
            ...
            .Radio(ID_VIEW_ZOOM_FIT, ID_VIEW_ZOOM_FIT, ID_VIEW_ZOOM_CUSTOM);
    ...
    void OnCommandOpen(HWND hwnd)
    {
        ...
        ofn.hwndOwner = hwnd;
        ofn.Flags = OFN_PATHMUSTEXIST | OFN_HIDEREADONLY | OFN_EXPLORER | OFN_ENABLESIZING;
        ofn.lpstrFile = path;
        ofn.nMaxFile = ARRAYSIZE(path);
        ofn.lpstrFilter = L"SVG files (*.svg;*.svgz)\0*.svg;*.svgz\0\0";
    ...
    void OnCommandSave(HWND hwnd)
    {
        if (this->viewer.IsLoaded()) {
            ...
            OPENFILENAMEW ofn = { sizeof(ofn) };
            ofn.hwndOwner = hwnd;
            ofn.Flags = OFN_PATHMUSTEXIST | OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER | OFN_ENABLESIZING;
            ofn.lpstrFile = path;
            ofn.nMaxFile = ARRAYSIZE(path);
    ...
    {
        bool shown = true;
        if (this->viewer.ShowViewBox(true)) {
            shown = false;
    ...
    {
        bool shown = true;
        if (this->viewer.ShowBoundingBox(true)) {
            shown = false;
